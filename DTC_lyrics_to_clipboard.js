...
(function(window, document) {
    ...
    window.dtcCopy = function(selector, button) {
        ...
        tempCopyArea.value = element.innerText;
    ...
    function createUI() {
        ...
        ui.id = UI_ID;
        ui.style.cssText = 'position:fixed;top:20px;right:20px;z-index:999999;background:rgba(0,0,0,0.9);color:#fff;padding:15px;border-radius:8px;font-family:sans-serif;box-shadow:0 4px 10px rgba(0,0,0,0.3);max-width:220px;';
        ...
        SELECTORS.forEach(selector => {
            ...
            button.style.cssText = 'display:block;width:100%;background:#333;color:#fff;border:1px solid #555;padding:8px;margin:5px 0;border-radius:4px;cursor:pointer;text-align:left;';
            button.textContent = selector;
            button.addEventListener('mouseover', () => {
                button.style.background = '#444';
                window.dtcHighlightElement(selector, true);
            ...
            button.addEventListener('click', () => {
                window.dtcCopy(selector, button);
            });
            ui.appendChild(button);
        });

        const footer = document.createElement('div');
        footer.style.cssText = 'font-size:11px;color:#999;margin-top:8px;text-align:center;';

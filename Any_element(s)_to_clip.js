...
javascript:(function(w, d) {
    ...
    // If the inspector UI already exists, clean it up and exit.
    ...
    // Handles modern navigator.clipboard API and falls back to a textarea method.
    const c2c = t => {
    ...
    // Keyboard event handler for Escape and Enter keys.
    const eH = e => {
        if (e.key === 'Escape') {
            cl();
            return;
        ...
        if (e.key === 'Enter') {
            ...
            const vB = Array.from(d.querySelectorAll(`#${I} button`)).filter(b => b.style.display !== 'none');
            if (vB.length === 0) return;
            const c = vB.map(b => E[b.getAttribute(D)]?.innerText).filter(txt => txt && txt.trim() !== '').join('\n\n');
            if (c) {
                c2c(c).then(() => {
    ...
    // Copy all content of a button's associated element.
    const cAA = (el, btn) => {
        if (!el) return;
    ...
    // Mouse click handler to copy element content.
    const pM = e => {
        const t = e.target;
        if (t.closest('#' + I) || !t.hasAttribute(D)) return;
        e.preventDefault();
        e.stopPropagation();
        const btn = d.querySelector(`#${I} button[${D}='${t.getAttribute(D)}']`);
        cAA(t, btn);
        return !1;
    };

    // Mouse hover handler to highlight elements and update the search bar.
    const hM = e => {
        const t = e.target;
        if (!t || t.closest('#' + I)) return;
        const sb = d.getElementById(SB);
        if (t.hasAttribute(D) && sb) {
            let txt = t.tagName.toLowerCase();
            if (t.id) txt += `#${t.id}`;
            if (t.className && typeof t.className === 'string') {
                const cl = t.className.trim().split(' ').filter(Boolean).join('.');
                if (cl) txt += `.${cl}`;
            }
            if (sb.value !== txt) {
                sb.value = txt;
        ...
        if (!t.hasAttribute(D)) return;
        hAH();
        const i = t.getAttribute(D),
            c = t.getAttribute(C);
        aH(t, c);
        const b = d.querySelector(`#${I} button[${D}='${i}']`);
        if (b) {
            b.classList.add(B);
    ...
    // Search filter function for the UI.
    const sF = e => {
        const v = e.target.value.toLowerCase();
        d.querySelectorAll(`#${I} button`).forEach(b => {
            const t = b.title.toLowerCase();
            b.style.display = t.includes(v) ? 'block' : 'none';
        });
    };

    // Main cleanup function to remove all inspector elements and event listeners.
    function cl() {
        const u = d.getElementById(I);
        if (u) u.remove();
        const s = d.getElementById('dtc-inspector-styles');
        if (s) s.remove();
        d.body.removeEventListener('mouseover', hM, !0);
        d.body.removeEventListener('click', pM, !0);
        d.removeEventListener('keydown', eH, !0);
        E.forEach(e => {
            rmH(e);
            e.removeAttribute(D);
            e.removeAttribute(C);
    ...
    // Injects the necessary CSS styles for the inspector.
    function iAS() {
        const id = 'dtc-inspector-styles';
        if (d.getElementById(id)) return;
        const s = d.createElement('style');
        s.id = id;
        s.textContent = `@keyframes dtc-glow-effect{0%{opacity:.8}100%{opacity:0;box-shadow:0 0 40px 15px var(--dtc-glow-color,#fff)}}.dtc-glow-animation{position:absolute;pointer-events:none;z-index:999999;border-radius:5px;animation:dtc-glow-effect .8s ease-out forwards}@keyframes dtc-btn-flash{0%,50%{background-color:#a6e22e!important;color:#000!important}100%{background-color:#333!important;color:#fff!important}}.${AC}{animation:dtc-btn-flash .5s ease-out}#${I} button:hover,#${I} .${B}{background-color:#5a5a5a!important;border-left:3px solid var(--dtc-btn-color, #fff)!important;}#${SB}{width:100%;box-sizing:border-box;padding:8px;margin-bottom:4px;background:#2a2a2a;border:1px solid #555;color:#fff;border-radius:4px;font:13px sans-serif;}`;
        d.head.appendChild(s);
    }

    // Plays a visual glow animation on an element.
    function pGA(el) {
        if (!el) return;
        const r = el.getBoundingClientRect(),
            g = d.createElement('div');
        g.className = 'dtc-glow-animation';
        const c = el.getAttribute(C) || 'rgba(255,255,255,0.8)';
        g.style.cssText = `left:${r.left+w.scrollX}px;top:${r.top+w.scrollY}px;width:${r.width}px;height:${r.height}px;--dtc-glow-color:${c};box-shadow:0 0 20px 8px ${c};`;
        d.body.appendChild(g);
        setTimeout(() => g.remove(), 800);
    }

    // Generates a random color for highlighting.
    function rC() {
        return `hsla(${~~(Math.random()*360)},100%,70%,0.8)`;
    }

    // Adds highlighting styles to an element.
    function aH(el, c) {
        if (!el) return;
        if (!el.hasAttribute(S)) {
            ...
            el.setAttribute(S, JSON.stringify(s));
        }
    ...
    // Removes highlighting styles from an element, restoring original styles.
    function rmH(el) {
        if (!el || !el.hasAttribute(S)) return;
        const s = JSON.parse(el.getAttribute(S));
        el.style.border = s.b;
        el.style.outline = s.o;
        el.style.boxShadow = s.x;
        el.removeAttribute(S);
        el.classList.remove(H);
    }

    // Removes all highlighting from all elements.
    function hAH() {
        d.querySelectorAll('.' + H).forEach(e => rmH(e));
        d.querySelectorAll('#' + I + ' .' + B).forEach(b => b.classList.remove(B));
    }

    // Creates the main inspector UI and appends it to the body.
    function cSUI() {
        ...
        t.style.cssText = 'font-size:14px;font-weight:bold;margin-bottom:10px;padding-bottom:5px;border-bottom:1px solid #555;';
        t.innerHTML = 'Element Inspector <span style="font-size:10px;font-weight:normal;color:#ccc;">(Esc to close)</span>';
        ui.appendChild(t);

        const sb = d.createElement('input');
        sb.id = SB;
        sb.type = 'text';
        sb.placeholder = 'Search or hover on elements...';
        sb.addEventListener('input', sF);
        ...
        n.style.cssText = 'font-size:12px;color:#ccc;margin-top:-2px;margin-bottom:8px;text-align:right;';
        n.innerHTML = 'copy filtered: <b style="color:#a6e22e;letter-spacing:1px;">enter</b>';
        ui.appendChild(n);

        // Find all visible elements on the page.
        const els = Array.from(d.querySelectorAll('body *')).filter(e => (e.offsetWidth > 0 || e.offsetHeight > 0) && w.getComputedStyle(e).visibility !== 'hidden');
        E = els;

        // Create a button for each element and add it to the UI.
        els.forEach((el, i) => {
            const c = rC();
            el.setAttribute(D, i);
            el.setAttribute(C, c);
            ...
            b.setAttribute(D, i);
            b.style.cssText = `display:block;width:100%;background:#333;color:#fff;border:0;border-left:3px solid transparent;padding:5px 8px;margin:2px 0;border-radius:3px;cursor:pointer;text-align:left;font:11px/1.4 monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:background .2s;--dtc-btn-color:${c};`;

            let txt = el.tagName.toLowerCase();
            if (el.id) txt += `<span style="color:#66d9ef;">#${el.id}</span>`;
            if (el.className && typeof el.className == 'string') txt += `<span style="color:#a6e22e;">.${el.className.trim().split(' ').join('.')}</span>`;
            b.innerHTML = txt;
            b.title = el.tagName.toLowerCase() + (el.id ? `#${el.id}` : '') + (el.className && typeof el.className == 'string' ? `.${el.className.trim().split(' ').join('.')}` : '');
    ...
    iAS();
    cSUI();
    d.body.addEventListener('mouseover', hM, !0);
    d.body.addEventListener('click', pM, !0);
    d.addEventListener('keydown', eH, !0);

    // Expose the cleanup function to the global scope for easy access.
